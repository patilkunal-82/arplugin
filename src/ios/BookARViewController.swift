    ...
    override func viewWillAppear(_ animated: Bool) {
       ...
       /* self.view.showBlurLoader()
        DispatchQueue.main.async {
            self.prepareAnchorsData()
            self.view.removeBlurLoader()
            self.runARWorldTracking()
            DispatchQueue.global(qos: .userInitiated).async {
                self.prepareContentsData()
            }
        }*/
...
extension BookARViewController {
    ...
    private func prepareAnchorsData() {
        print("!!!! prepare anchor")
        ...
        for anchor in bookAnchorContentNames.keys {
            ...
            //print("anchors data \(anchorsAndContentData)")
        }
        //print("FINAL anchors data \(anchorsAndContentData)")
        prepareContentsData()
       
    }
    
    private func prepareContentsData() {
        print("!!!! prepare content")
        
        for arRefImage in anchorsAndContentData.keys {
            for key in bookAnchorContentNames.keys {
                
                if arRefImage.name == key {
                    
                 var contentArray: [AssetContainer] = []
                 for content in bookAnchorContentNames[arRefImage.name!] ?? [] {
                     let path = content
                ...
                // print("anchorsAndContentData \(anchorsAndContentData)")
        }
      }
        ...
        print("FINALanchorsAndContentData \(anchorsAndContentData)")
        self.view.removeBlurLoader()
    }
...
class BlurLoader: UIView {
    ...
    private func addLoader() {
        ...
        loadingTextLabel.text = "Preparing to immerse you.."
        loadingTextLabel.font = UIFont(name: "Avenir Light", size: 16)
        loadingTextLabel.sizeToFit()
        loadingTextLabel.center = CGPoint(x: activityIndicator.center.x, y: activityIndicator.center.y - 40)
